...
ros::Publisher pubLaserCloud_rawData;
...
*/
 ...
 // cout<<"z: "<<accZ<<endl;
  ...
  //ROS_WARN("imu ShiftXYZ");
  //cout<<"shift x : "<< imuShiftX[imuPointerLast]<<endl;
 //cout<<"shift y : "<< imuShiftY[imuPointerLast]<<endl;
  //cout<<"shift z : "<< imuShiftZ[imuPointerLast]<<endl;
}
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn2)
{
  ...
  laserRawData.header.frame_id = "/camera_init";
  ...
  //ROS_WARN("check start and end orientation");
  ...
  { 
    ...
    point.x = laserCloudIn->points[i].x;//y;
    point.y = laserCloudIn->points[i].y;//z;
    point.z = laserCloudIn->points[i].z;//x;

    //cout<<point.x<<" , "<<point.y<<" , "<<point.z<<endl;
    //float angle = atan(point.y / sqrt(point.x * point.x + point.z * point.z)) * 180 / PI;
    float angle = atan(point.z / sqrt(point.x * point.x + point.y * point.y)) * 180 / PI;
...
//      y++;
    ...
    //cout<<angle<<endl;
    //ROS_WARN("check fxxk scanID");
   // cout<<"scanID : "<<scanID<<endl;
...
//    }
    ...
    point.intensity = scanID + 0.05 * relTime;//0.1
/*
    if (imuPointerLast >= 0) 
    {
  ...
  for (int i = 5; i < cloudSize - 6; i++) 
  {
        ...
        if (diff > 0.05) //0.1
        {
            ...
            if (depth1 > depth2) 
            {
                  ...
                  if (sqrt(diffX * diffX + diffY * diffY + diffZ * diffZ) / depth2 < 0.05) //0.1
                  {
            ...
            else 
            {
                  ...
                  if (sqrt(diffX * diffX + diffY * diffY + diffZ * diffZ) / depth1 < 0.05)//0.1 
                  {
                        cloudNeighborPicked[i + 1] = 1;
                        cloudNeighborPicked[i + 2] = 1;
                        cloudNeighborPicked[i + 3] = 1;
  ...
  for (int i = 0; i < 16; i++) 
  {
      ...
      for (int j = 0; j < 6; j++) 
      {
            ...
            for (int k = ep; k >= sp; k--) 
            {
                  ...
                  if (cloudNeighborPicked[ind] == 0 && cloudCurvature[ind] > 0.05) //0.05
                  {
                        ...
                        else if (largestPickedNum <= 30) //30
                        {
                          cloudLabel[ind] = 1;
                          cornerPointsLessSharp->push_back(laserCloud->points[ind]);
                        } 
            ...
            for (int k = sp; k <= ep; k++) 
            {
                  ...
                  if (cloudNeighborPicked[ind] == 0 &&
                      cloudCurvature[ind] < 0.05)//0.1 //0.25
                  {
    ...
    downSizeFilter.setLeafSize(0.3, 0.3, 0.3); //Outdoor
    //downSizeFilter.setLeafSize(0.2, 0.2, 0.2); //Intdoor Test
  ...
  laserCloud2.header.frame_id = "/camera_init";
  ...
  cornerPointsSharp2.header.frame_id = "/camera_init";
  ...
  cornerPointsLessSharp2.header.frame_id = "/camera_init";
...
/*
  ...
  imuTrans->points[0].x = imuRollStart;//imuPitchStart;
  imuTrans->points[0].y = imuPitchStart;//imuYawStart;
  imuTrans->points[0].z = imuYawStart;//imuRollStart;

  imuTrans->points[1].x = imuRollCur;//imuPitchCur;
  imuTrans->points[1].y = imuPitchCur;//imuYawCur;
  imuTrans->points[1].z = imuYawCur;//imuRollCur;
  ...
  */
  ...
  for (int i = 0; i < 16; i++) {
    //ROS_WARN("scan register, show size ...");
    //cout<<i<<"\n"<<laserCloudScans[i]->points.size()<<endl;
    laserCloudScans[i]->points.clear();
  }
 // ROS_INFO ("Released");

    double time_end =  ros::Time::now().toSec();
    //ROS_WARN("scan register ");
...
void imuHandler(const sensor_msgs::Imu::ConstPtr& imuIn)
{
  ...
  Eigen::Quaternionf altitude_imu;
  altitude_imu.w() = imuIn->orientation.w;
  altitude_imu.x() = imuIn->orientation.x;
  altitude_imu.y() = imuIn->orientation.y;
  altitude_imu.z() = imuIn->orientation.z;

  Eigen::Matrix3f Rot_imu;
  Rot_imu = altitude_imu;
  ...
  acc_w = Rot_imu * acc_w - gravity;

//  float accX = imuIn->linear_acceleration.x + sin(pitch) * 9.78;
//  float accY = imuIn->linear_acceleration.y - sin(roll) * cos(pitch) * 9.78;
//  float accZ = imuIn->linear_acceleration.z - cos(roll) * cos(pitch) * 9.78;
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> 
                                  ("/velodyne_points", 10, laserCloudHandler);
