...
void transformAssociateToMap()
{
  ...
  float tmp_pitch = atan2(srycrx / cos(transformMapped[0]), crycrx / cos(transformMapped[0]));
  
  if(fabs(tmp_pitch-transformMapped[1])>=pi_round){
    if(tmp_pitch<0)
      tmp_pitch += 2 * M_PI;
    else
      tmp_pitch -= 2 * M_PI;
  }      
  transformMapped[1] = tmp_pitch;
  ...
  float tmp_yaw = atan2(srzcrx / cos(transformMapped[0]), crzcrx / cos(transformMapped[0]));
     
  if(fabs(tmp_yaw-transformMapped[2])>=pi_round){
    if(tmp_yaw<0)
      tmp_yaw += 2 * M_PI;
    else
      tmp_yaw -= 2 * M_PI;
