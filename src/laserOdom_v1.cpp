...
void TransformToStart(pcl::PointXYZI *pi, pcl::PointXYZI *po)
{
  ...
  if(isnan(po->x)||isnan(po->y)||isnan(po->z))
  {
    ROS_WARN("##########################################");
...
{
  float s = 20 * (pi->intensity - int(pi->intensity));//10
...
void viconCallBack(const geometry_msgs::PoseStamped::ConstPtr &vicon_odom )
{
    ...
    vicon_odom_.header.frame_id = "/camera_init";
    ...
    path_vicon.header.frame_id = "/camera_init";
...
int main(int argc, char** argv)
{
  ...
  ros::Rate rate(100);// should be 50 ???
  bool status = ros::ok();
  while (status) {
    ros::spinOnce();
    ...
    if (newCornerPointsSharp && newCornerPointsLessSharp && newSurfPointsFlat && 
      ...
      if (laserCloudCornerLastNum > 10 && laserCloudSurfLastNum > 100) {
        ...
        for (iterCount = 0; iterCount < 15; iterCount++) // 25
        {
          ...
          if (fabs(matX.at<float>(0, 0)) < 0.05 &&
          fabs(matX.at<float>(1, 0)) < 0.05 &&
          fabs(matX.at<float>(2, 0)) < 0.05 &&
          fabs(matX.at<float>(3, 0)) < 0.1 &&
          fabs(matX.at<float>(4, 0)) < 0.1 &&
          fabs(matX.at<float>(5, 0)) < 0.1) {
...
*/
      ...
      laserOdometry.pose.pose.orientation.x = geoQuat.x;//-geoQuat.y;//-geoQuat.y;
      laserOdometry.pose.pose.orientation.y = geoQuat.y;//-geoQuat.z;//-geoQuat.z;
      laserOdometry.pose.pose.orientation.z = geoQuat.z;//geoQuat.x;
